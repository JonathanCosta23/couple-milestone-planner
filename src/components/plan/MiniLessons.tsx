...
export function MiniLessons() {
  ...
  const { user } = useAuth();
  const { isCompleted, isOpened, completedCount, markOpened, markCompleted } =
    useEducationProgress(user?.id);

  const handleToggle = (lesson: MiniLesson) => {
    const next = expandedId === lesson.id ? null : lesson.id;
    setExpandedId(next);
    if (next) void markOpened(lesson.id);
  };
    ...
    <div className="space-y-4">
      <Card className="glass-card-strong p-4 text-center">
        ...
        <p className="text-xs text-muted-foreground mt-1">
          Lições rápidas sobre o que realmente importa para seu dinheiro crescer
        </p>
        {user && (
          <p className="text-[11px] text-muted-foreground mt-2">
            {completedCount} de {MINI_LESSONS.length} concluídas
          </p>
        )}
      </Card>
