...
export function SubNav({ items, active, onChange }: Props) {
  return (
    <div className="-mx-4 sm:-mx-6 lg:-mx-8 relative">
      ...
      <div className="flex gap-1.5 lg:gap-2 overflow-x-auto lg:overflow-x-visible pb-2 scrollbar-hide snap-scroll-x px-4 sm:px-6 lg:px-8 lg:flex-wrap">
        {items.map(item => (
          <Button
            key={item.id}
            size="sm"
            variant={active === item.id ? "default" : "outline"}
            className={`text-xs shrink-0 lg:shrink h-9 rounded-full px-3.5 lg:px-4 touch-target transition-all ${
              active === item.id
                ? "shadow-sm"
